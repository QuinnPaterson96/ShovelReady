...
Use one fresh sheet per person and build. Record a later improved-build retest separately, with its own identities and prior-exposure note. Never combine baseline and retest task times or success counts. The [frozen agent rehearsal](baseline-agent-rehearsal.md) is attributed **agent evidence**, not a human sheet. Agent wall time is not a human completion-time measure. Preserve its original observations and any later adjudication as separate entries; do not infer human behavior or general accuracy from it.
